...
def rent_591_object_list(argu):
    ...
    params = {}
    for part in argu_content.split('&'):
        if '=' in part:
            k, v = part.split('=', 1)
            params[k] = v
    ...
    req = session.get(_SEARCH_URL, params=params, timeout=10)
    data = req.json()
...
def rent_591_object_list_tostring(items):
    ...
    for item in items:
        ...
        price = item['price'] + ' ' + item.get('price_unit', '元/月')
        link = item.get('url', 'https://rent.591.com.tw/{}'.format(item['id']))
